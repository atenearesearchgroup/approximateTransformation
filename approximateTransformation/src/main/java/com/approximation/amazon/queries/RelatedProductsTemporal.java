...
/**
 * Class RelatedProductsTemporal
 */
public class RelatedProductsTemporal implements Runnable {
	...
	private final static Logger LOGGER = Logger.getLogger(RelatedProductsTemporal.class.getName());

	/**
	 * Query RelatedProducts for temporal approximation
	 * 
	 ...
	 * 
	 */
	public RelatedProductsTemporal(TinkerGraph graph, Boolean creation, Long windowI, Long windowE) {
		this.graph = graph;
		...
		while (i < 6) {
			...
			if (!creation) {
				...
				List<Collection<Object>> result = graph.traversal().V().hasLabel("Product").as("product1")
						.in("contains").where(__.values("date").is(P.inside(windowI, windowE))).as("order1")
				...
				graph.traversal().V().hasLabel("Product").as("product1").in("contains")
						.where(__.values("date").is(P.inside(windowI, windowE))).as("order1").out("contains")
